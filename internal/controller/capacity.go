...
func (r *InstasliceReconciler) actualProfileSliceSize(ctx context.Context, instaslice *inferencev1alpha1.Instaslice, profileName, gpuUUID string) (int32, error) {
	updatedInstaslice, err := r.getInstasliceObject(ctx, instaslice.Name, instaslice.Namespace)
	if err != nil {
		return 0, err
	}
	gpuAllocatedIndex := gpuAllocatedSlices(updatedInstaslice, gpuUUID)
	actualSliceSize := int32(0)
	startIdx := r.getStartIndexFromPreparedState(updatedInstaslice, profileName, gpuAllocatedIndex)
	// Get required slice size
	var neededContinousSlot int32
	for profile, placement := range updatedInstaslice.Status.NodeResources.MigPlacement {
		if profile == profileName {
			neededContinousSlot = placement.Placements[0].Size
			break
		}
